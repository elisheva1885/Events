...
import { EventsEntity, GetSupplierRecommendationsAction } from "@/product-types";
...
import { formatEventDate, getEventStatusProgress } from "@/utils/DateUtils";
...
export default function MyEvents() {
  ...
  const { data: events, isLoading } = useEntityGetAll(EventsEntity, {
    clientEmail: user.email,
  });

  const { createFunction, isLoading: isCreating } = useEntityCreate(EventsEntity);
  const { updateFunction, isLoading: isUpdating } = useEntityUpdate(EventsEntity);
  const { deleteFunction, isLoading: isDeleting } = useEntityDelete(EventsEntity);
